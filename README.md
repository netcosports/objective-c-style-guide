...
Conditional bodies should always use braces even when a conditional body could be written without braces (e.g., it is one line only) to prevent errors. These errors include adding a second line and expecting it to be part of the if-statement. Another, хeven more [dangerous defect](http://programmers.stackexchange.com/a/16530) may happen where the line “inside” the if-statement is commented out, and the next line unwittingly becomes part of the if-statement. In addition, this style is more consistent with all other conditionals, and therefore more easily scannable.
...
If it's possible try to reduce nesting.
...
When it comes to the variable qualifiers [introduced with ARC](https://developer.apple.com/library/ios/releasenotes/objectivec/rn-transitioningtoarc/Introduction/Introduction.html#//apple_ref/doc/uid/TP40011226-CH1-SW4), the qualifier (`__strong`, `__weak`, `__unsafe_unretained`, `__autoreleasing`) should be placed before the variable type for better readability, e.g., `__weak NSString *text`. 
...
```objc
static const NSTimeInterval NSTArticleViewControllerNavigationFadeAnimationDuration = 0.3f;
```
...
```objc
@interface UIViewController (NSTMediaPlaying)
...
@interface NSString (NSTAdditions)
```
 ...
 * Method explanation.
 * 
 * @param alpha Represents transparency of an object. It varies between 0.0f and 1.0f.
 */
- (void)doSomethingWithAlpha:(CGFloat)alpha
{
...
- (instancetype)init
{
    // Call super initializer or the designated initializer.
    self = [super init];
    if (self) {
